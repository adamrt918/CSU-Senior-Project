...
bool ChoicePage::loadMedia(SDL_Renderer* gRenderer, int pgNum){
    switch (pgNum){
        ...
        case GAME_PAGE_6:
            ...
            if (!textures[0].loadFromRenderedText(gRenderer, GAME_PAGE_6_WORDS[0], GREY, dimensions.w() / 1.3))
            {
                printf( "Failed to render text texture!\n" );
                return false;
            }          
    ...
    if (gamer->getInsane()){
        ...
        if (!textures[1].loadFromRenderedText(gRenderer, choice[chooser()].text, TAN, dimensions.w() / 1.3))
        {
            printf( "Failed to render text texture!\n" );
            return false;
        }
    ...
    switch (currentPage){
        case GAME_PAGE_2:
            if (!gamer->getInsane()){
                ...
                if (textures[1].isMouseOver(textures[1].getRect())){
                    *textColor = GREY;
                    ...
                    { 
                        ...
                        //Set the new value with the change in stats
                        gamer->setHealth(gamer->getHealth() + choice[chooser()].statChange[0]);
                        gamer->setSanity(gamer->getSanity() + choice[chooser()].statChange[1]);
                        gamer->setRep(gamer->getRep() + choice[chooser()].statChange[2]);
                    }
                }
                else{
                    *textColor = TAN;
                ...
                textures[1].loadFromRenderedText(renderer, choice[chooser()].text, *textColor, dimensions.w()/1.3);
            } // End insane events
            break;
        case GAME_PAGE_6:
                        if (!gamer->getInsane()){
